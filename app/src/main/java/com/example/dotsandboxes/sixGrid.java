...
public class sixGrid extends View {
...
int boxNumber=0;
    ...
    boolean verticalLine [][]=new boolean[6][6];
    boolean horizontalLine [][]=new boolean[6][6];
    int playerNo=0;
    int player1score=0;
    ...
    public sixGrid(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public  sixGrid(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
            ...
            if (xTouch <=getWidth()/2f+400  && yTouch <=getHeight()/2f+400 ) {
                ...
                for (int i = 0; i < 6; i++) {
                    for (int j = 0; j < 6; j++) {
                        ...
                        if (xTouch >= cx && xTouch <= cx +160 && yTouch >= cy - 40 && yTouch <= cy + 40) {
                            /*horizontal line*/

                            if(!horizontalLine[i][j])   {
                                horizontalLine[i][j]=true;
                                Log.i(TAG,"i j is"+i+j);
                                lineContainer.add(new Line(cx,cy,cx+160,cy,playerNo));
                                int x=i,y=j,flag=1;
                                if(y+1<=5){
                                    ...
                                    if(horizontalLine[x][y]&&verticalLine[x][y]&&horizontalLine[x][y+1]&&verticalLine[x+1][y]){
                                        ...
                                        boxContainer.add(new Boxes(cx+10,cy+10,cx+160-10,cy+160-10,color) );
                                        playerNo++;
                                        playerNo%=2;
                                        flag=0;
                                    }
                        ...
                        } else if (xTouch >= cx - 40 && xTouch <= cx + 40 && yTouch >= cy && yTouch <= cy+160) {
                            /*vertical line*/
                            if(!verticalLine[i][j]){
                                verticalLine[i][j]=true;
                                lineContainer.add(new Line(cx,cy,cx,cy+160,playerNo));
                                int x=i,y=j,flag=1;
                                if(x-1>=0){
                                    if(playerNo==0)color=Color.RED;
                                    else color=Color.BLUE;
