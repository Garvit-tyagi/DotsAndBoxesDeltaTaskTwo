...
public class fourGrid extends View {
   int boxNumber=0;
    float xTouch, yTouch;
    ...
    boolean verticalLine [][]=new boolean[4][4];
    boolean horizontalLine [][]=new boolean[4][4];
    int playerNo=0;
    int player1score=0;
    ...
    public fourGrid(Context context) {
        super(context);
        init(null);
    }

    public fourGrid(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public  fourGrid(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (xTouch <=getWidth()/2f+390  && yTouch <=getHeight()/2f+390 ) {
                ...
                for (int i = 0; i < 4; i++) {
                    for (int j = 0; j < 4; j++) {
                        ...
                        if (xTouch >= cx && xTouch <= cx +260 && yTouch >= cy - 40 && yTouch <= cy + 40) {
                            /*horizontal line*/

                            if(!horizontalLine[i][j])   {
                                horizontalLine[i][j]=true;
                                Log.i(TAG,"i j is"+i+j);
                                lineContainer.add(new Line(cx,cy,cx+260,cy,playerNo));
                                int x=i,y=j,flag=1;
                                if(y+1<=3){
                                    ...
                                    if(horizontalLine[x][y]&&verticalLine[x][y]&&horizontalLine[x][y+1]&&verticalLine[x+1][y]){
                                        ...
                                        boxContainer.add(new Boxes(cx+10,cy+10,cx+260-10,cy+260-10,color) );
                                        playerNo++;
                                        playerNo%=2;
                                        flag=0;
                                    }
                        ...
                        } else if (xTouch >= cx - 40 && xTouch <= cx + 40 && yTouch >= cy && yTouch <= cy+260) {
                            /*vertical line*/
                            if(!verticalLine[i][j]){
                                verticalLine[i][j]=true;
                                lineContainer.add(new Line(cx,cy,cx,cy+260,playerNo));
                                int x=i,y=j,flag=1;
                                if(x-1>=0){
                                    if(playerNo==0)color=Color.RED;
                                    else color=Color.BLUE;
